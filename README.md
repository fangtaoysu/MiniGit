...
一个基于 DDD 架构实现的简化版 Git。命令来自控制台输入：
  - 命令识别：借鉴编译器编译的思路，分词 → 状态机 → 分发执行

## 架构总览
- Presentation：负责字符串命令的分词、存储与命令分发，不含业务流程与复杂领域逻辑。
...
## 命令处理管线（编译同款）
- 词法分析（Lexer）：把输入字符串流切分为 Token（命令、子命令、参数、选项）。
...
- 特别说明：
  - 文件名：全部用小写，下划线连接，C++ 文件要以 .cc 结尾, 头文件以 .h 结尾.
  - 类名：使用驼峰命名法，首字母大写（如 `ConfigReader`）。
  - 函数名：使用驼峰命名法，首字母大写（如 `ReadConfig`）。
  - 变量名：全部用小写，下划线连接（如 `config_reader`）。
  - 常量名：命名时以 “k” 开头, 大小写混合（如 `kMaxRetries`）。
  - 命名空间：全部用小写，下划线连接（如 `mini_git`）。
  - 枚举：单独的枚举值应该优先采用 常量 的命名方式（如 `kMergeStrategy`）。
