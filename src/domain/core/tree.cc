...
Tree::Tree() : obj_type_("tree"), entries_{} { sha1_ = CalculateSha1(); }

Tree Tree::AddEntry(const std::string& mode, const std::string& obj_type,
...
std::string Tree::CalculateSha1() const {
    ...
    for (const auto& entry : entries_) {
        data += entry.mode + " " + entry.obj_type + " " + entry.sha1 + "\t" +
                entry.name + "\n";
    }
...
std::string Tree::GetSha1() const { return sha1_; }
...
std::string Tree::Deserialize(std::span<const uint8_t> data) {
    ...
    while (pos < s.size()) {
        ...
        std::string obj_type =
            left.substr(first_space + 1, second_space - first_space - 1);
