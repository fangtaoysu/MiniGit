...
protected:
    static void SetUpTestSuite() {
        ...
        bool config_loaded =
            AppConfig::GetInstance().LoadConfig(CONFIG_FILE_PATH);
        ...
        ASSERT_TRUE(db_config.enable)
            << "Database is not enabled in the test config.";
...
TEST_F(DatabaseManagerTest, ShouldExecuteUpdate) {
    DbManager::GetInstance().Execute(
        "INSERT INTO test_users (name, email) VALUES (?, ?)",
        {"initial_name", "update@example.com"});

    // 更新
    int64_t affected_rows = DbManager::GetInstance().Execute(
        "UPDATE test_users SET name = ? WHERE email = ?",
        {"updated_name", "update@example.com"});
    ASSERT_EQ(affected_rows, 1);
...
TEST_F(DatabaseManagerTest, ShouldRollbackTransaction) {
    ...
    DbManager::GetInstance().Execute(
        "INSERT INTO test_users (name, email) VALUES (?, ?)",
        {"tx_rollback", "rollback@example.com"});
...
TEST_F(DatabaseManagerTest, ShouldHandleInvalidQueryGracefully) {
    // 这个查询会因为表名错误而失败
    auto results =
        DbManager::GetInstance().Query("SELECT * FROM non_existent_table");
...
TEST_F(DatabaseManagerTest, ShouldHandleInvalidExecuteGracefully) {
    // 这个执行会因为语法错误而失败
    int64_t affected_rows = DbManager::GetInstance().Execute(
        "INSERTT INTO test_users (name) VALUES ('fail')");
    // 期望返回 -1，而不是抛出异常或崩溃
    ASSERT_EQ(affected_rows, -1);
}